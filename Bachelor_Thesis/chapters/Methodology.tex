...
\begin{enumerate}
    \item \textbf{FR-01 User Authentication.}
          ...
          local-identity middleware that validates the \texttt{X-User-Email} header
    ...
    \item \textbf{FR-05 Messaging.}
          ...
          group chats.
          Both conversation types shall support file attachments up to 20~MB.
...
\begin{enumerate}
    ...
    \item \textbf{NFR-02 Security.}
          All API endpoints shall require a valid \texttt{X-User-Email} header,
          validated by local-identity middleware.
...
Task~Flow is structured as a three-tier desktop application embedded in a Tauri shell.
...
hub (\texttt{/hubs/notifications}), and six background services.
...
\textbf{Tauri Shell.}
The Tauri main process (\texttt{src-tauri/src/lib.rs}) launches the compiled .NET
backend as a sidecar process via the shell plugin~\cite{tauri_sidecar}.
It monitors the child's standard output stream and waits for the line
\texttt{TASKFLOW\_BACKEND\_READY:\{address\}} before closing the splashscreen
window and revealing the main application window.
...
React component $\to$ \texttt{fetch()} with \texttt{X-User-Email}
header $\to$ Kestrel $\to$ local-identity middleware resolves user and creates
\texttt{ClaimsPrincipal} $\to$
controller action $\to$ service $\to$ EF Core $\to$ SQLite.
If the operation mutates a syncable entity, the service additionally calls
\texttt{IMirrorService.}\cbrk\texttt{MirrorAsync()}, which either performs a direct MongoDB upsert
...
\texttt{BCrypt.Verify()} and, on success, returns the user's identity
information including their \texttt{AppUser} record.
No token is issued; the session is managed by local-identity middleware
that reads the \texttt{X-User-Email} header on each request and constructs
a \texttt{ClaimsPrincipal} with the user's identifier, email, and display name
from the SQLite database.\footnote{This approach was chosen over JWT to
eliminate token issuance, refresh, and revocation complexity in a
single-user desktop context.}
A fire-and-forget call to \texttt{UserDataSyncService.}\cbrk\texttt{PullForUserAsync()} pulls the
...
Password reset uses a locally generated recovery code: an 8-character
alphanumeric token is written to a temporary file in the system's temp
directory (\texttt{\%TEMP\%/taskflow\_reset\_pending.tmp}) and read by the
Tauri main process via IPC (\texttt{invoke("read\_reset\_code")}).
The reset code is valid for 15~minutes; the reset endpoint validates the
code, checks expiry, and updates the password hash in both SQLite and
the MongoDB credential backup.
...
the \texttt{ClaimsPrincipal}'s \texttt{NameIdentifier} claim (set by the
local-identity middleware), returning a 403 response for
...
\texttt{user\_\{userId\}}.
The user identity is resolved by the local-identity middleware during the
WebSocket upgrade handshake; the \texttt{Context.UserIdentifier} property
reflects the \texttt{NameIdentifier} claim set by the middleware.
The unread notification count is pushed to the client immediately upon connection.

The system defines seventeen notification types across four priority levels.
...
nullable fields on the \texttt{Message} entity and are limited to 20~MB by Kestrel's
...
It runs a 30-second ping loop: on each tick it calls \texttt{MongoService.}\cbrk\texttt{PingAsync()}
and updates \texttt{ConnectivityService.}\cbrk\texttt{IsOnline}.
...
then calls \texttt{Replay}\cbrk\texttt{Outbox}\cbrk\texttt{Async()}.
...
\texttt{HttpCompletionOption.}\cbrk\texttt{ResponseHeadersRead}, then processes the response body as
...
\texttt{Response.}\cbrk\texttt{WriteAsync()}.
...
three components: the Tauri shell, the compiled .NET backend as a sidecar, and the
Webpack-built React frontend assets.

\textbf{Tauri shell.}
The desktop application is built with Tauri v2~\cite{tauri_docs}, a Rust-based framework
that uses the operating system's native WebView (Microsoft WebView2 on Windows) rather
than bundling a browser engine.
The Rust source code in \texttt{src-tauri/src/} is organised into \texttt{lib.rs}
(plugin registration, window lifecycle, and application setup), \texttt{backend.rs}
(sidecar spawning), and \texttt{commands.rs} (IPC commands).

\textbf{Sidecar backend management.}
The ASP.NET Core backend is declared as a sidecar via \texttt{externalBin} in
\texttt{tauri.conf.json}~\cite{tauri_sidecar}.
On startup, \texttt{spawn\_backend()} in \texttt{backend.rs} sets environment variables
(\texttt{ASPNETCORE\_ENVIRONMENT}, \texttt{ASPNETCORE\_URLS}) and launches the process.
The backend announces its dynamically assigned port by printing
\texttt{TASKFLOW\_BACKEND\_}\cbrk\texttt{READY:\{address\}} to standard output.
The Rust main process parses stdout, stores the URL in shared managed state
(\texttt{BackendUrl}), and signals the frontend via IPC.

\textbf{IPC interface.}
Nine \texttt{\#[tauri::command]} functions bridge the native shell and the web
renderer: \texttt{get\_backend\_url}, \texttt{get\_startup\_status},
\texttt{read\_reset\_code} (for password recovery), \texttt{get\_version},
\texttt{get\_app\_data\_dir}, \texttt{open\_file\_dialog} (placeholder), and three
window management commands (\texttt{window\_minimize}, \texttt{window\_maximize},
\texttt{window\_close}) for a custom title-bar implementation.

\textbf{Window lifecycle.}
Two windows are declared: \texttt{splashscreen} (small, undecorated, shown while the
backend starts) and \texttt{main} (the full application window).
Both start invisible; the splashscreen is shown during backend startup, and when the
backend signals ready (\texttt{TASKFLOW\_BACKEND\_}\cbrk\texttt{READY}), the URL is stored,
the main window is revealed, and the splashscreen is closed.

\textbf{Tauri build configuration.}
\texttt{tauri.conf.json} configures the target artefacts:
\begin{itemize}
    \item \textbf{Windows} --- NSIS installer with \texttt{currentUser} install mode
          (no administrator privileges required) and MSI installer.
    \item \textbf{WebView2} --- \texttt{downloadBootstrapper} mode downloads and
          installs the WebView2 runtime if absent; on Windows~10 (version~1803 and
          later) and Windows~11, WebView2 is already integrated into the operating
          system~\cite{webview2}.
\end{itemize}

The build is orchestrated by \texttt{BUILD\_WINDOWS.bat}, which runs \texttt{npm ci},
TypeScript type checking (\texttt{npx tsc --noEmit}), Webpack~5 frontend build,
\texttt{dotnet publish} (self-contained, \texttt{win-x64}), and \texttt{npx tauri build}
to produce the final installer.
The resulting installer is self-contained: no .NET runtime, Node.js, or Rust toolchain
...
Local-identity middleware resolution and bcrypt hash/verify round-trips were tested in isolation.

\textbf{API integration testing.}
Each of the 14 controllers was exercised against a running Kestrel instance using
\texttt{HttpClient} requests with a valid \texttt{X-User-Email} header, a missing
header, and a non-existent email.
The tests verified: (a)~that the local-identity middleware resolves the user correctly
in all three cases (falling back to the first database user when the header is
absent); (b)~that ownership checks return~403 for resources belonging to a different
user; and (c)~that create, read, update, and delete operations produce the expected HTTP
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Tauri Shell Architecture}\label{sec:meth_tauri}

This section documents the Tauri v2~\cite{tauri_docs}
desktop shell that packages the ASP.NET Core backend and the React frontend
into a native Windows installer.

\subsection{Tauri Project Structure}\label{sec:meth_tauri_structure}

The Tauri shell resides in \texttt{src-tauri/} and organises its Rust
source into three modules: \texttt{lib.rs} (plugin registration, setup, and
window lifecycle), \texttt{backend.rs} (sidecar spawning and termination), and
\texttt{commands.rs} (IPC commands callable from the frontend).

\subsection{Plugin Architecture}\label{sec:meth_tauri_plugins}

Six Tauri plugins are registered: \texttt{shell} (sidecar lifecycle),
\texttt{single-instance} (prevents duplicate instances), \texttt{log},
\texttt{dialog} (native file dialogs), \texttt{fs} (file access for
password-reset codes and the application data directory), and \texttt{http}
(reserved for future use).

\subsection{Sidecar Backend Management}\label{sec:meth_tauri_sidecar}

The ASP.NET Core backend is declared as a sidecar in
\texttt{tauri.conf.json} via \texttt{externalBin: ["binaries/taskflow-backend"]}~\cite{tauri_sidecar}.
During the build, the compiled .NET executable receives the target-triple
suffix (e.g.\ \texttt{taskflow-backend-}\cbrk\texttt{x86\_64-pc-windows-msvc.exe}) and is placed in
\texttt{src-tauri/binaries/}.

\texttt{spawn\_backend()} in \texttt{backend.rs} sets environment variables
(\texttt{ASPNETCORE\_ENVIRONMENT}, \texttt{ASPNETCORE\_URLS}) and launches the process---via
\texttt{tokio::process::Command} in development, or the shell plugin's
\texttt{sidecar()} method in production.
Stdout is parsed line-by-line for the \texttt{TASKFLOW\_BACKEND\_READY}
signal; the dynamically assigned backend URL is stored in shared
managed state (\texttt{BackendUrl}) and exposed to the frontend via IPC.
A clean shutdown kills the child process on \texttt{ExitRequested} to prevent
orphaned backends.

\subsection{IPC Command Implementation}\label{sec:meth_tauri_ipc}

Nine \texttt{\#[tauri::command]} functions are defined in
\texttt{commands.rs} and invoked from the frontend via
\texttt{invoke()}\cite{tauri_docs}.
\texttt{get\_backend\_url} and \texttt{get\_startup\_status} expose the
backend's dynamically assigned port and startup phase to the splashscreen;
\texttt{read\_reset\_code} bridges the password-reset flow to the desktop
context; \texttt{get\_version} and \texttt{get\_app\_data\_dir} provide
package metadata and the platform-specific data directory;
\texttt{open\_file\_dialog} is a placeholder for native file selection;
and three window commands (\texttt{window\_minimize},
\texttt{window\_maximize}, \texttt{window\_close}) implement a custom
title-bar with native controls.

\subsection{Security Configuration}\label{sec:meth_tauri_security}

Tauri v2's security model centres on a Content Security Policy (CSP) and a
declarative capabilities system.
The CSP restricts connections to \texttt{http://127.0.0.1:*} and
\texttt{ws://127.0.0.1:*} (for the local backend and its SignalR hub), while
allowing external fonts from Google Fonts.
The capabilities file (\texttt{capabilities/default.json}) grants per-plugin
permissions granularly, e.g.\ \texttt{fs:allow-read-text-file} for password
reset codes and \texttt{shell:allow-open} for opening URLs in the system
browser.

\subsection{Window Lifecycle and Splashscreen}\label{sec:meth_tauri_window}

Two windows are declared: \texttt{splashscreen} (small, undecorated) and
\texttt{main} (full application window), both initially invisible.
The \texttt{setup()} closure in \texttt{lib.rs} manages shared state
(\texttt{BackendUrl}, \texttt{StartupStatus}, \texttt{BackendProcess}), shows
the splashscreen, and calls \texttt{spawn\_backend()}.
Once \texttt{handle\_ready\_line()} receives the port announcement, it stores
the URL, reveals the main window, and closes the splashscreen.
On exit, \texttt{stop\_backend()} kills the sidecar, preventing orphaned
processes.

\subsection{Build Pipeline}\label{sec:meth_tauri_build}

The production build is orchestrated by \texttt{BUILD\_WINDOWS.bat}, which
executes six sequential steps:

\begin{enumerate}
    \item \textbf{Dependency installation.} \texttt{npm ci} installs
          frontend dependencies deterministically from
          \texttt{package-lock.json}.
    \item \textbf{Type checking.} \texttt{npx tsc --noEmit} verifies
          TypeScript correctness.
    \item \textbf{Frontend build.} \texttt{npm run build:web} executes
          Webpack~5 to produce production-optimised bundles in
          \texttt{wwwroot/dist/}, including the splashscreen HTML page.
    \item \textbf{Backend publish.} \texttt{dotnet publish} compiles the
          ASP.NET Core backend as a self-contained single-file executable
targeting \texttt{win-x64}.
    \item \textbf{Sidecar staging.} The published backend executable is
renamed to \texttt{taskflow-backend-}\cbrk\texttt{x86\_64-pc-windows-msvc.exe}
           and placed in \texttt{src-tauri/binaries/}.
    \item \textbf{Tauri build.} \texttt{npx tauri build} compiles the Rust
          shell, bundles the frontend assets, embeds the sidecar, and
          produces an NSIS installer targeted at
          \texttt{x86\_64-pc-windows-msvc}.
\end{enumerate}

The resulting installer is self-contained: no .NET runtime, Node.js, or Rust
toolchain is required on the target machine.
WebView2 is handled by the Tauri installer configuration
(\texttt{downloadBootstrapper} mode), which downloads and installs the
WebView2 runtime if it is not already present on the system.
On Windows~10 (version~1803 and later) and Windows~11, WebView2 is already
integrated into the operating system, so the bootstrapper step is typically
skipped~\cite{webview2}.
