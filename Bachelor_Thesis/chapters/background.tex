...
For Task~Flow, the ASP.NET Core backend is bundled as a \emph{sidecar}
...
Electron and Tauri.
The Task~Flow desktop shell uses Tauri v2, chosen for its significantly smaller
...
used by Task~Flow's backend. Its provider model allows the same application
...
accessible only from the local desktop application, Task~Flow does not
