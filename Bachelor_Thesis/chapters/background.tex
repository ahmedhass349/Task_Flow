...
This chapter provides the background context for the design and implementation of
TaskFlow, then surveys the academic literature and technical foundations that support
its key architectural decisions.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Background of Task Management}\label{sec:bg_background}
% ─────────────────────────────────────────────────────────────────────────────

Task management is the process of tracking, organising, and prioritising work items
through their lifecycle from creation to completion.
It is a fundamental activity in nearly every professional domain: software developers
manage feature requests and bug fixes, healthcare workers track patient follow-ups,
educators coordinate curriculum milestones, and enterprise teams break large projects
into assignable units of work.

Historically, task management relied on physical artifacts---paper to-do lists,
whiteboards, sticky notes, and wall calendars.
David Allen's \emph{Getting Things Done}~(GTD)~\cite{allen2001gtd} formalised this
into a personal productivity methodology that externalises tasks into a trusted system,
freeing cognitive resources for execution rather than recall.
The Kanban system~\cite{anderson2010kanban}, originating in Toyota's manufacturing
process, introduced visual workflow management through physical cards moved across
columns representing production stages.

The transition from physical to digital tools brought significant advantages:
instant replication, search, reminders, and collaboration across geographic boundaries.
However, the first generation of digital tools inherited an implicit assumption from
their server-based architectures: that a reliable network connection is always
available for reading and writing data.
This assumption, while reasonable when the tools were designed, is increasingly at
odds with modern work patterns that span trains, aeroplanes, co-working spaces, and
rural locations with intermittent connectivity.

The limitations of this cloud-first model---coupled with the fragmentation of
task management across multiple messaging and productivity applications---motivate
the need for a desktop task management system that functions reliably offline,
synchronises transparently when connectivity is restored, and consolidates
collaboration features into a single interface.
The following section evaluates how well current commercial tools address these needs.
...
\emph{Getting Things Done}~(GTD), which argues that externalising
...
natural language text, source code, or ordered tokens of any kind --- without relying on
recurrent or convolutional structure. Introduced by Vaswani et~al.~\cite{vaswani2017attention}
in the 2017 paper \textit{Attention Is All You Need}, it replaced recurrent
sequence-to-sequence frameworks with a purely attention-based mechanism.
The core innovation is the \emph{self-attention} operation: each token in an input
sequence computes a weighted sum of \emph{every} other token's representation, allowing
the model to capture long-range dependencies directly rather than compressing the entire
sequence through a fixed-size hidden state as recurrent networks must. Because
self-attention processes all positions simultaneously rather than stepping through tokens
one by one, training can be parallelised across the sequence length, enabling orders of
magnitude larger datasets and deeper models than were feasible with recurrent
architectures. This parallelisation and the ability to propagate gradients across
arbitrary distances in a single layer made the Transformer the foundation for all
subsequent Large Language Models (LLMs) such as GPT-4, Claude, and Llama.
...
mechanisms.
Mistral offers multiple model tiers optimised for different tasks:
\texttt{mistral-small-latest} for general conversational use,
\texttt{codestral-latest} for code generation, and
\texttt{mistral-ocr-latest} for document and image text extraction.
