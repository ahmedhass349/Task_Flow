...
However, several directions for improvement and extension were identified
during development.
...
(\texttt{TASKFLOW\_}\cbrk\texttt{MISTRAL\_KEY}) at startup, falling back to the
...
\begin{enumerate}
  ...
  \item \textbf{End-to-end tests} using Playwright to exercise the packaged
        desktop application, verifying that key user journeys (login, create task, go offline,
...
\paragraph{Cross-platform Tauri builds.}
The current Tauri build pipeline targets only Windows (NSIS and MSI).
Extending the build to macOS and Linux would restore cross-platform
coverage. The Rust source code in
\texttt{src-tauri/} is already platform-independent; the main effort lies
in configuring the build pipeline for each target triple
(\texttt{aarch64-apple-darwin}, \texttt{x86\_64-unknown-linux-gnu}) and
adapting the \texttt{BUILD\_WINDOWS.bat} script into a cross-platform CI
pipeline. Once macOS and Linux builds are available, Apple notarisation would
eliminate the Gatekeeper manual-override step described in
Section~\ref{sec:res_distribution}.
...
creation and use the AI assistant to speed up workflows beyond
question answering.
...
A future SaaS variant would replace the embedded Tauri desktop shell with a web-hosted
