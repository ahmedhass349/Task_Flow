...
Table~\ref{tab:mongo_appendix} lists the MongoDB collections used as the relay
and mirror layer (fifteen in total).
...
\addlinespace
\texttt{cross\_}\cbrk\texttt{notifications}
  & Cross-device notification relay for multi-machine alerts.
    Fields: recipientEmail, title, message, notificationType,
    relatedTaskId, createdAt. \\
\addlinespace
\texttt{user\_accounts}
  & Encrypted password-hash backup for account restoration after reinstall.
    Fields: email, passwordHash, userId, backedUpAt. \\
\addlinespace
\texttt{team\_}\cbrk\texttt{announcements}
  & Team-wide announcement storage. Fields: teamId, authorEmail, title,
    message, createdAt, readBy. \\
\addlinespace
\texttt{app\_settings}
  & Key-value configuration store. Documents keyed by \texttt{\_id}. \\
\addlinespace
\texttt{users}
  & Mirror of \texttt{AppUser} profile data (Company, Country, Phone,
    Timezone). Fields: email, fullName, avatarUrl, company, country,
    phone, timezone. \\
\addlinespace
\texttt{teams}
  & Mirror of \texttt{Team} entity. Fields: name, description, color,
    ownerId, createdAt. \\
\addlinespace
\texttt{team\_member\_}\cbrk\texttt{records}
  & Mirror of per-member team metadata (distinct from
    \texttt{team\_members}). Fields: teamId, userId, role, joinedAt. \\
\addlinespace
\texttt{messages}
  & Mirror of \texttt{Message} entity. Fields: senderId, receiverId,
    text, fileUrl, mimeType, fileSize, readAt, soft-delete flags. \\
\addlinespace
\texttt{chatbot\_}\cbrk\texttt{conversations}
  & Mirror of AI chatbot conversation records.
    Fields: userId, title, mode (General/Coder/Scanner), createdAt. \\
\addlinespace
\texttt{chatbot\_messages}
  & Mirror of individual AI chat messages.
...
\midrule
\texttt{productName}  & \texttt{TaskFlow} \\
